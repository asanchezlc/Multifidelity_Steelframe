...
# --- Symbols ---
E, I, Lc = sym.symbols('E I Lc', positive=True, finite=True)
n_levels = 5

Kyy_column, _ = outils.assemble_global_beam_sym(E, I, Lc, n_levels)  # one column
Kyy_column, _ = outils.reduce_cantilever_sym(Kyy_column)
rot_dofs = [i for i in range(Kyy_column.shape[0]) if i % 2 == 1]
Kyy_column, _ = outils.static_condensation(Kyy_column, slave_dofs=rot_dofs)
