...
from helpers.classes import StoryLevelRigidX
...
K_upd = outils.compute_Y_torsional_stiffness_frame_rigid_X(K, levels)
a, b = outils.generalized_eig_singular(M, K_upd)

base_name = filename_json.replace('.json', '')
np.savetxt(os.path.join(output_path, f'{base_name}_K.txt'), K_upd, fmt="%.6e", delimiter="\t")
